...
class ExampleFreezedBloc
  ...
  FutureOr<void> _addName(
    ...
    final currentNames = state.maybeWhen(
      data: (names) => names,
      orElse: () => const <String>[],
    );

    emit(ExampleFreezedState.showBanner(names: currentNames, message: 'Aguarde, nomes sendo inseridos'));
    await Future.delayed(const Duration(seconds: 2));

    final updatedNames = [...currentNames, event.name];
    emit(ExampleFreezedState.data(names: updatedNames));
  }
  ...
  FutureOr<void> _removeName(
    ...
    final updatedNames = [...currentNames, event.name];
    updatedNames.retainWhere((element) => element != event.name);
    emit(ExampleFreezedState.data(names: updatedNames));
  }
}
